...
typedef struct _no_cir_dup_enc {
	int linha;
	int coluna;
	int valor;
	struct _no_cir_dup_enc *prox;
	struct _no_cir_dup_enc *ant;
} NoCDup;

typedef struct _lista_cir_dup_enc {
	NoCDup *inicio;
	NoCDup *fim;
} ListaCDup;

NoCDup *CriaNoCDup (int l, int c, int v){
	NoCDup *p = (NoCDup*) malloc(sizeof(NoCDup));
	p->linha = l;
	p->coluna = c;
	p->valor = v;
	p->prox = p->ant = p;
...
ListaCDup *CriaListaCDup(){
	ListaCDup *L = (ListaCDup*) malloc(sizeof(ListaCDup));
	L->inicio = L->fim = NULL;

	return L;
}

int VerificarSeListaCDupEstaVazia(ListaCDup *L){
	return (L->inicio == NULL);
}

void InsereNoFimDaListaCDup(ListaCDup *L, int l, int c, int v){
	NoCDup *p = CriaNoCDup(l,c,v);
	//Caso 1: Lista está vazia:
	if (VerificarSeListaCDupEstaVazia(L)){
		L->inicio = L->fim = p;
	}
...
void RemoveNoCDup(ListaCDup *L, int l, int c){
	//A lista possui elementos.
	if (!VerificarSeListaCDupEstaVazia(L)){
		NoCDup *p = L->inicio;
		//Caso 1: O nó a ser removido é a cabeça da lista.
		if ((L->inicio->linha == l)&&(L->inicio->coluna == c)){
		...
		//Caso 2: elemento está no meio da lista / o nó não é a cabeça da lista
		else{
			p=p->prox;
			while(p != L->inicio){
				if((p->linha == l)&&(p->coluna == c)){
					p->ant->prox = p->prox;
					p->prox->ant = p->ant;
...
void ImprimeListaCDup(ListaCDup *L){
	if(VerificarSeListaCDupEstaVazia(L)){
		puts("\tJogadas realizadas: ");
		puts("\tNenhuma jogada realizada!");
	}
	else{
		NoCDup *p = L->inicio;
...
int VerificarSeAPosicaoExisteNaLista(ListaCDup *L, int l, int c){
	if(!VerificarSeListaCDupEstaVazia(L)){
		NoCDup *p = L->inicio;
		do{
			if((p->linha == l)&&(p->coluna == c)){
...
void TrocarValorDaPosicaoListada(ListaCDup *L, int l, int c, int v){
	NoCDup *p = L->inicio;
...
void VerificarValor(lista *inicio, ListaCDup *jogadas, int matriz[][DIM], int linha, int coluna, int valor){
	NoCDup *p = jogadas->inicio;
	if (valor == 0){
		system("clear");
		puts("\tO valor da posição informada foi apagado.\n");
		TrocarValor(matriz,linha,coluna,88);
		RemoveNoCDup(jogadas,linha,coluna);
	}
	else if((valor>0)&&(valor<10)){
		system("clear");
		if(valor == BuscaDeElementos(inicio,valor)){
			TrocarValor(matriz,linha,coluna,valor);
			if (VerificarSeAPosicaoExisteNaLista(jogadas, linha, coluna) == 1){
				TrocarValorDaPosicaoListada(jogadas,linha,coluna,valor);
			}
			else{
				InsereNoFimDaListaCDup(jogadas, linha, coluna, valor);
			}
...
void main (){
	...
	ListaCDup *ListaJogadas = CriaListaCDup();
	...
	while(!VerificacaoDaMatriz(matrizsudoku,vetor)){
		...
		do{
			setbuf(stdin,NULL);
			printf("\n\tInforme o valor (OBS: para apagar o valor já existente, informe o valor 0): ");
			scanf("%d",&valor);
			if((valor<0) || (valor>9)){
		...
		VerificarValor(inicio,ListaJogadas,matrizsudoku,linha,coluna,valor);
		ImprimeListaCDup(ListaJogadas);
